...
/// composition layer (`package:build_engine/game.dart`,
/// `src/plugins/game/run_events.dart`), not in `technique_plugin.dart`
/// — the Technique plugin itself has no such event; `evolveTechnique`
/// just returns an `EvolutionResult`. Publishing `TechniqueEvolved` is
/// something a caller does explicitly after resolving an evolution
/// (mirroring `training_stage.dart`'s own `events.publish(...)` call),
/// so any later adapter that evolves a technique must publish this same
/// event class for `EngineSession`'s subscription below to see it.
class EngineSession {
